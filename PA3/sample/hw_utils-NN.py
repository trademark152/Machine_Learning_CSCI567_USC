        ...
        for reg_coeff in reg_coeffs:
            for sgd_decay in sgd_decays:
                for sgd_mom in sgd_moms:
                    ...
                    probability = model.predict_proba(X_te, batch_size=batch_size, verbose = verbose)
                    if score[1] > best_acc:
                        best_acc = score[1]
                        best_config = [arch, reg_coeff, sgd_decay, best_acc]
...
def testmodels_test(X_tr, y_tr, X_te, archs, actfn='relu', last_act='softmax', reg_coeffs=[0.0], 
				num_epoch=30, batch_size=1000, sgd_lr=1e-5, sgd_decays=[0.0], sgd_moms=[0.0], 
					sgd_Nesterov=False, EStop=False, verbose=0):
    [n_tr,d] = X_tr.shape
    [n_te,d] = X_te.shape
