...
def hmm_test():
    ...
    # print("test obs_dict: ", obs_dict)

    # state symbols
    states_symbols = dict()
    for idx, item in enumerate(data['states']):
        states_symbols[item] = idx
    # print("test states_symbols: ", states_symbols)
...
def speech_tagging_test():
    ...
    # read data from given files
    data = Dataset("tags.txt", "sentences.txt", train_test_split=0.8, seed=0)

    # initialize part of train data
    data.train_data = data.train_data[:100]

    # initialize part of test data
    data.test_data = data.test_data[:10]

    # # train model on these data
    # for trainData in data.train_data:
    #     trainData.show()
